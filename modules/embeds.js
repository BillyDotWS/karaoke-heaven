...
module.exports.infoEmbed = class {
	...
	async sendEmbed() {
		...
		else if (this.personToTag === null) {this.embedPlaceholder = await this.channel.send(infoEmbed).then(msg => msg.delete({ timeout: this.timer * 1000 }));}
	...
	editEmbed() {
		...
		else if (this.personToTag === null) {this.channel.edit(infoEmbed).then(msg => msg.delete({ timeout: this.timer * 1000 }));}
