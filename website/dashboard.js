...
module.exports = (client) => {
	...
	const publicDir = path.resolve('./templates/public/');
  ...
  */
		...
		if (req.isAuthenticated()) {
			...
			for(guild in req.user.guilds) {
				if(req.user.guilds[guild].id == "700208007530676314") {
					...
					renderTemplate(res, req, 'events.ejs', { req: req }, { eventlist: eventlist }, { Discord: Discord }, { clickHandler:'func1();' });
					test = true
		...
		if (req.isAuthenticated()) {
			...
			for(guild in req.user.guilds) {
				if(req.user.guilds[guild].id == "700208007530676314") {
					...
					renderTemplate(res, req, 'banking.ejs', { req: req }, { eventlist: eventlist }, { Discord: Discord }, { clickHandler:'func1();' });
					test = true
	...
	app.get('/formtest', (req, res) => {
		renderTemplate(res, req, 'blocks/modals/eventcreator.ejs', { md });
	});
