		...
		if(announcetime <= 900) {
			...
			if(eventinfo[0].announced == false) {
				...
				if(`${eventinfo[0].category}` === "Official") {
					...
					await main.client.message.guild.channels.cache.get("700209781859942400").fetch(`${eventinfo[0].embedid}`).fetch()
					const shrugUsers = main.client.message.guild.channels.cache.get("700209781859942400").fetch(`${eventinfo[0].embedid}`).reactions.cache.get(`711061304240242738`).users.cache.map(users => users.id);
